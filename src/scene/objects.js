...
import { buildTV } from "../components/tv.js";
...
function buildDesk() {
  ...
  for (let row = 0; row < 4; row++) {
    for (let col = 0; col < 12; col++) {
      ...
      key.position.set(-0.21 + col * 0.038, 0.896, -0.02 + row * 0.038);
      group.add(key);
    }
  }
...
function buildBookshelf() {
  ...
  for (const sy of shelfYs) {
    const shelf = new THREE.Mesh(
      new THREE.BoxGeometry(1.15, 0.03, 0.28),
      shelfMat,
    );
    shelf.position.y = sy;
  ...
  const bookColours = [0x00d4ff, 0x00ff88, 0x00d4ff, 0xe0e8f0, 0x111828];
  for (const sy of shelfYs) {
    let xOffset = -0.5;
    for (let i = 0; i < 10; i++) {
  ...
  const pacShape = new THREE.SphereGeometry(
    0.08,
    16,
    12,
    0.3,
...
function buildPedestal() {
  ...
  const leftPage = new THREE.Mesh(
    new THREE.BoxGeometry(0.18, 0.01, 0.22),
    pageMat,
  );
...
function buildRabbitHole() {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    tuft.position.set(Math.cos(angle) * 0.4, 0.04, Math.sin(angle) * 0.4);
...
function buildGlobe() {
  ...
  gctx.beginPath();
  gctx.moveTo(100, 40);
  gctx.quadraticCurveTo(130, 35, 140, 60);
  gctx.quadraticCurveTo(150, 90, 130, 110);
  gctx.quadraticCurveTo(110, 120, 95, 100);
  gctx.quadraticCurveTo(80, 70, 100, 40);
  gctx.fill();
  // South America
  gctx.beginPath();
  gctx.moveTo(140, 130);
  gctx.quadraticCurveTo(160, 140, 155, 180);
  gctx.quadraticCurveTo(145, 210, 130, 200);
  gctx.quadraticCurveTo(120, 170, 140, 130);
  gctx.fill();
  // Europe
  gctx.beginPath();
  gctx.moveTo(250, 45);
  gctx.quadraticCurveTo(280, 40, 290, 60);
  gctx.quadraticCurveTo(285, 80, 265, 85);
  gctx.quadraticCurveTo(245, 75, 250, 45);
  gctx.fill();
  // Africa
  gctx.beginPath();
  gctx.moveTo(260, 90);
  gctx.quadraticCurveTo(290, 95, 285, 140);
  gctx.quadraticCurveTo(275, 180, 255, 175);
  gctx.quadraticCurveTo(240, 145, 260, 90);
  gctx.fill();
  // Asia
  gctx.beginPath();
  gctx.moveTo(310, 35);
  gctx.quadraticCurveTo(380, 30, 400, 60);
  gctx.quadraticCurveTo(420, 90, 400, 110);
  gctx.quadraticCurveTo(360, 120, 330, 100);
  gctx.quadraticCurveTo(300, 80, 310, 35);
  gctx.fill();
  // Australia
  gctx.beginPath();
  gctx.ellipse(400, 175, 25, 18, 0, 0, Math.PI * 2);
  gctx.fill();
  // Greenland
  gctx.beginPath();
  gctx.ellipse(170, 30, 18, 12, 0.3, 0, Math.PI * 2);
  gctx.fill();
...
function buildRadio() {
  ...
  const body = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.22, 0.15), bodyMat);
...
function createMusicNotes(scene) {
  ...
  function update(delta, elapsed) {
    for (const note of notes) {
      if (active) {
        note.material.opacity = Math.min(
          note.material.opacity + delta * 2,
          0.8,
        );
        note.position.y += note.userData.speed * delta;
        note.position.x =
          note.userData.baseX +
          Math.sin(elapsed * 2 + note.userData.phase) * 0.15;
...
export function createObjects(scene) {
  ...
  const posters = [
    buildAIPoster(-2.5, 2.0, -2.9, "NEURAL\nNETWORKS", "brain", 0),
    buildAIPoster(-1.4, 2.0, -2.9, "MACHINE\nLEARNING", "eye", 1),
    buildAIPoster(-0.3, 2.0, -2.9, "SYNTHETIC\nSTORYTELLERS", "circuit", 2),
  ];
  ...
  bookshelf.userData = {
    ...
    action: "openPanel",
    panelId: "bookshelf",
    panelTitle: "Game Library",
  };
  fridge.userData = {
    clickable: true,
    action: "openPanel",
    panelId: "fridge",
    panelTitle: "Refreshments",
  };
  floorLamp.userData = {
    clickable: true,
    action: "openPanel",
    panelId: "lamp",
    panelTitle: "Mood Lighting",
  };
  neonSign.userData = {
    clickable: true,
    action: "openPanel",
    panelId: "sign",
    panelTitle: "Welcome to the Game Room",
  };
